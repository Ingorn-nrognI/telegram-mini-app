...
function r(e) {
	...
	}).then((e) => new Uint8Array(e)), t.catch(() => n.delete(e)), n.set(e, t)), t;
}
function i(e) {
	return n.has(e);
}
...
}, s = new o(), c = new URL("" + new URL("tlottie.wasm", import.meta.url).href, "" + import.meta.url), l = 0;
function u() {
	return typeof crypto < "u" && typeof crypto.randomUUID == "function" ? crypto.randomUUID() : (l += 1, `tlottie-warmup-${Date.now()}-${l}`);
}
function d(e = {}) {
	let t = e.pool ?? (e.workerCount === void 0 ? s : new o(e.workerCount)), n = (e.wasmUrl ?? c).toString(), r = t.getAllWorkers();
...
function f(e) {
	let t = `url("data:image/svg+xml;base64,${p(e)}")`;
	return {
		maskImage: t,
		WebkitMaskImage: t
	};
...
var h = /* @__PURE__ */ new Map(), g = typeof IntersectionObserver > "u" ? null : new IntersectionObserver((e) => {
	for (let t of e) {
		let e = t.target.dataset.tlottieId;
		e && h.get(e)?.setObservable(t.isIntersecting);
	}
}), _ = 0;
function v() {
	return typeof crypto < "u" && typeof crypto.randomUUID == "function" ? crypto.randomUUID() : (_ += 1, `tlottie-${Date.now()}-${_}`);
}
var y = class {
	id = v();
	state = "loading";
	...
	constructor(e) {
		this.config = e, this.canvas = e.canvas, this.pool = e.pool ?? (e.workerCount === void 0 ? s : new o(e.workerCount)), requestAnimationFrame(() => {
	...
	destroy() {
		this.destroyed || (this.worker?.postMessage({
			...
			id: this.id,
			action: "destroy"
		}), this.destroyed = !0, this.worker?.removeEventListener("message", this.onMessage), this.worker = null, this.resizeObserver?.disconnect(), this.resizeObserver = null, this.resizeRaf && cancelAnimationFrame(this.resizeRaf), g?.unobserve(this.canvas), h.delete(this.id), this.listeners.clear());
	}
	start() {
		this.canvas.dataset.tlottieId = this.id;
		let e = this.canvas.transferControlToOffscreen();
		this.worker = this.pool.getWorker(), this.worker.addEventListener("message", this.onMessage), h.set(this.id, this), g?.observe(this.canvas), typeof ResizeObserver < "u" && (this.resizeObserver = new ResizeObserver(() => this.scheduleResize()), this.resizeObserver.observe(this.canvas));
	...
	onMessage = (e) => {
		...
		if (t.type !== "warmed" && t.type !== "warmup-error" && t.id === this.id) switch (t.type) {
			case "meta":
				...
				break;
			case "event":
				this.frames = t.frames, this.state = b(t.event, this.state), this.emit(t.event, { frames: t.frames });
				break;
...
//#endregion
export { t as DEFAULT_RENDER_QUALITY, e as FitzModifier, y as TLottie, o as TLottieWorkerPool, f as buildShimmerMaskStyle, m as configureTLottie, s as defaultWorkerPool, d as initializeTLottie, i as isAnimationCached };
